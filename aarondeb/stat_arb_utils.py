...
def run_ou_tests(prices_df: pd.DataFrame, combinations: list, test_period: str = '2Y', cross_overs_per_delta: int = 12, num_threads: int = 8, verbose: bool = True) -> pd.DataFrame:
    """
...
def _outer_cointegration_loop(prices_df: pd.DataFrame, molecule: list) -> pd.DataFrame:
    ...
    for pair in molecule:
        cointegration_results.append(coint(prices_df.loc[:, pair[0]], prices_df.loc[:, pair[1]], trend="c", method="aeg", maxlag=None, autolag="aic",
                                           return_results=None))

    return pd.DataFrame(cointegration_results, index=molecule, columns=['coint_t', 'pvalue', 'crit_value'])
